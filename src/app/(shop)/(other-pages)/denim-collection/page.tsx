...
export const metadata: Metadata = {
  title: 'Collection Denim Collection',
  description: 'Classic and modern denim styles for every wardrobe.',
}

export default async function DenimCollectionPage() {
  const collection = (await getCollectionByHandle('denim-collection')) || (await getCollectionByHandle('bootcut')) || (await getCollectionByHandle('all'))
  ...
  return (
    <div className="container">
      <div>
        <Breadcrumb breadcrumbs={breadcrumbs} currentPage={'Denim Collection'} className="py-3.5" />
        <Divider />
        <main className="">
          <div className="flex flex-col items-center py-14 text-center lg:py-20">
            ...
            <Heading bigger level={1} className="mt-5">
              ...
              <br />
              <span data-slot="italic" className="underline">Denim Collection</span>
            </Heading>
            <Text className="mt-5 max-w-xl">Classic and modern denim styles for every wardrobe.</Text>
          </div>
            ...
            <section>
              <div className="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-10 lg:grid-cols-3 lg:gap-x-7 xl:grid-cols-4">
                {products.map((product) => (
                  <ProductCard key={product.id} product={product} showVendor={true} labelOverride="DENIM" />
                ))}
              </div>
