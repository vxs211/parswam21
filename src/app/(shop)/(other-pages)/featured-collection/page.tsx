...
export const metadata: Metadata = {
  title: 'Collection Featured Collection',
  description: 'Handpicked highlights of the week.',
}

export default async function FeaturedCollectionPage() {
  const collection = (await getCollectionByHandle('featured-collection')) || (await getCollectionByHandle('all'))
  ...
  return (
    <div className="container">
      <div>
        <Breadcrumb breadcrumbs={breadcrumbs} currentPage={'Featured Collection'} className="py-3.5" />
        <Divider />
        <main className="">
          <div className="flex flex-col items-center py-14 text-center lg:py-20">
            ...
            <Heading bigger level={1} className="mt-5">
              ...
              <br />
              <span data-slot="italic" className="underline">Featured Collection</span>
            </Heading>
            <Text className="mt-5 max-w-xl">Handpicked highlights of the week.</Text>
          </div>
            ...
            <section>
              <div className="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-10 lg:grid-cols-3 lg:gap-x-7 xl:grid-cols-4">
                {products.map((product) => (
                  <ProductCard key={product.id} product={product} showVendor={true} labelOverride="FEATURED" />
                ))}
              </div>
