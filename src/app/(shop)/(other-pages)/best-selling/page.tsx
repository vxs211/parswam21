...
export const metadata: Metadata = {
  title: 'Collection Best Selling',
  description: 'Our most-loved styles and top sellers.',
}

export default async function BestSellingPage() {
  const collection = (await getCollectionByHandle('best-selling')) || (await getCollectionByHandle('all'))
  ...
  return (
    <div className="container">
      <div>
        <Breadcrumb breadcrumbs={breadcrumbs} currentPage={'Best Selling'} className="py-3.5" />
        <Divider />
        <main className="">
          <div className="flex flex-col items-center py-14 text-center lg:py-20">
            ...
            <Heading bigger level={1} className="mt-5">
              ...
              <br />
              <span data-slot="italic" className="underline">Best Selling</span>
            </Heading>
            <Text className="mt-5 max-w-xl">Our most-loved styles and top sellers.</Text>
          </div>
            ...
            <section>
              <div className="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-10 lg:grid-cols-3 lg:gap-x-7 xl:grid-cols-4">
                {products.map((product) => (
                  <ProductCard key={product.id} product={product} showVendor={true} labelOverride="BEST SELLING" />
                ))}
              </div>
