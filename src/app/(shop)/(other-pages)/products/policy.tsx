...
export interface Props {
  shippingPolicy: TPolicy
  refundPolicy: TPolicy
  subscriptionPolicy: TPolicy
}

const Policy: FC<Props> = ({ shippingPolicy, refundPolicy, subscriptionPolicy }) => {
  const A_FEATURES = [
    {
      color: 'bg-red-50',
      ...
      {A_FEATURES.map((item, index) => {
        return (
          <Link
            key={`${index + 1}`}
            href={item.learnMore}
            className={`relative flex flex-col rounded-2xl p-5 ${item.color} dark:bg-opacity-90 group`}
          >
            <span className="absolute end-5 top-5 opacity-60 transition-all group-hover:scale-110 group-hover:opacity-100">
            ...
            <div className="mt-4">
              ...
              <div className="mt-1 line-clamp-1 text-sm text-slate-500">{item.desc}</div>
            </div>
          </Link>
        )
      })}
