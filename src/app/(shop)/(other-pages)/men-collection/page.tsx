...
export const metadata: Metadata = {
  title: 'Collection Men Collection',
...
export default async function MenCollectionPage() {
  const collection = (await getCollectionByHandle('men-collection')) || (await getCollectionByHandle('relaxed-fit')) || (await getCollectionByHandle('all'))
  ...
  return (
    <div className="container">
      <div>
        <Breadcrumb breadcrumbs={breadcrumbs} currentPage={'Men Collection'} className="py-3.5" />
        <Divider />
        <main className="">
          <div className="flex flex-col items-center py-14 text-center lg:py-20">
            ...
            <Heading bigger level={1} className="mt-5">
              ...
              <br />
              <span data-slot="italic" className="underline">Men Collection</span>
            </Heading>
            <Text className="mt-5 max-w-xl">Premium men's jeans with perfect fit. Timeless styles for every occasion.</Text>
          </div>
            ...
            <section>
              <div className="grid grid-cols-1 gap-y-4 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-10 lg:grid-cols-3 lg:gap-x-7 xl:grid-cols-4">
                {products.map((product) => (
                  <ProductCard key={product.id} product={product} showVendor={true} labelOverride="MEN" />
                ))}
              </div>
